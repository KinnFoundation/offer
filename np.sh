...
EOF
	...
	compile () {
		echo "${REACH_VERSION}"
		REACH_DEBUG=YES REACH_VERSION="${REACH_VERSION}" ./reach compile ${infile:-index}.rsh --install-pkgs
		REACH_DEBUG=YES REACH_VERSION="${REACH_VERSION}" ./reach compile ${infile:-index}.rsh "${@}"
	}
	eject () {
		_ () {
			node <(connector "${1}")
...
EOF
	...
	v2-register () {
		curl -vvv -X POST "${API_ENDPOINT_TESTNET}/api/v2/register" -H 'Content-Type: application/json' -H "Authorization: Bearer ${ZB_AUTH_TOKEN}" -d @<( eject )
	}
	v2-launch () {
		local plan_id="${1}" 
